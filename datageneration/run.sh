...
#JOB_PARAMS=${1:-'--idx ${idx} --name ${name} --ishape 0 --stride 50'}
...
do
    echo ${i}
    echo ${array[$i]}
    JOB_PARAMS="--idx ${i} --name ${array[$i]} --ishape 0 --stride 50"
    echo $JOB_PARAMS
#    ### RUN PART 1  --- Uses python3 because of Blender
    nohup $BLENDER_PATH/blender -b -t 6 -P main_part1.py --- ${JOB_PARAMS} > out.out
        #
#    ### RUN PART 2  --- Uses python2 because of OpenEXR
#    PYTHONPATH="" ${PYTHON2_PATH}/bin/python2.7 main_part2.py ${JOB_PARAMS}
done
