...
do
    ...
    echo $JOB_PARAMS
    nohup $BLENDER_PATH/blender -b -t 4 -P main_part1.py --- ${JOB_PARAMS} > "${subject_id}_${direction}.out"
    mv "/home/local/data/cmc/synthetic/run0/${name}/${name}_c0001.mp4" "/home/local/data/cmc/synthetic/run0/${name}/${name}_f.mp4"

    direction="backward"
    JOB_PARAMS="--idx ${i} --name ${name} --ishape 0 --stride 50 --subject_id ${subject_id} --direction ${direction}"
    echo $JOB_PARAMS
    nohup $BLENDER_PATH/blender -b -t 4 -P main_part1.py --- ${JOB_PARAMS} > "${subject_id}_${direction}.out"
    mv "/home/local/data/cmc/synthetic/run0/${name}/${name}_c0001.mp4" "/home/local/data/cmc/synthetic/run0/${name}/${name}_b.mp4"
done
"
